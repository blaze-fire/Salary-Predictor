...
class Preprocess:
    ...
    def education(self, df):
        ...
        for j in range(len(df)):
            ...
            if re.findall(r'(graduate|bachelor|master)', df['requirements'][j].lower()):
                education_list.append(re.search(r'(graduate|bachelor|master)', df['requirements'][j].lower()).group())

            else:
                education_list.append('education_missing')
    ...
    def seniority(self, df):
        ...
        for j in range(len(df)):
            ...
            if re.findall(r'(junior|senior)', df['requirements'][j].lower()):
                seniority_list.append(re.search(r'(junior|senior)', df['requirements'][j].lower()).group())

            else:
                seniority_list.append('seniority_missing')
    ...
    def analyze_skills(self, df):
        ...
        skills_list = []
        with open('./utils/skills.txt', 'r') as f:
            skills = f.read()
        ...
        for j in range(len(df)):
            ...
            for skill in skills_list:
                ...
                if re.findall(skill, df['requirements'][j].lower()):
                    length = len(re.findall(skill, df['requirements'][j].lower()))

                    if skill not in df.columns:
                        df[skill] = np.zeros(len(df), dtype='int')

                    df.loc[j, skill] += length

                if re.findall(skill, df['Job_position'][j].lower()):
                    length = len(re.findall(skill, df['Job_position'][j].lower()))
